...
def vcalling(VAR, MM):
    ...
    TSV = [ALT_COUNT, DP_HQ, AB, REFt, ALT_COUNT_p, ALT_COUNT_padj, STRAND, FISHER, ALT_COUNT_o, OUT_adj]
...
TSV_HEADER = ['CHROM', 'POS', 'REF', 'ALT', 'DP_HQ', 'REFt', 'ALT_COUNT', 'AB', 'P_VAL', 'P_VAL_adj', 'STRAND', 'FISHER', 'ALT_COUNT_o', 'P_VALo_adj', 'FILTER']
...
with open(FILE) as f1:
    for i in f1:
        ...
        if line.startswith('##'):
            ...
            filter = []
            for VAR in CALL:
                refVAR, altVAR, tsvVAR, filterVAR = vcalling(VAR, MM)
            ...
            if (len(ref) > 1):
                ...
                for count in range(0,n):
                    ...
                    if len(refi) != len(REF):
                         # Sample info
                        ALT_COUNT, DP_HQ, AB, REFt, ALT_COUNT_p, ALT_COUNT_padj, STRAND, FISHER, ALT_COUNT_o, OUT_adj = TSV[count]
                ...
                # TSV line
                TSV_LINE = [str(CHROM), str(POS), str(ref[0]), str(alt[0]), str(DP_HQ), str(REFt), str(ALT_COUNT), str(AB), ALT_COUNT_p, ALT_COUNT_padj, STRAND, FISHER, ALT_COUNT_o, OUT_adj, FILTER]
                ...
                # Sample info
                ALT_COUNT, DP_HQ, AB, REFt, ALT_COUNT_p, ALT_COUNT_padj, STRAND, FISHER, ALT_COUNT_o, OUT_adj = TSV[0]
