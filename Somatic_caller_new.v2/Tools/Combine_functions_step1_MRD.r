...
COUNT <- function(DP1,DP2,DP3,DP4){
  ...
  # Variant covered
  ALL <- (rbind(unique1, unique2, unique3, unique4))
  ALL <- ALL[nchar(ALL$REF) == 1,]
  ALL <- unique(ALL)
...
for (i in (colnames(FINAL)[c(5:ncol(FINAL))])){
  FINAL[,i] <- as.numeric(as.character(FINAL[,i]))
}

FINAL$POS <- as.numeric(as.character(FINAL$POS))

FINAL$AB <- FINAL$ALT_COUNT/FINAL$DP_HQ

# Likely somatic
if (nrow(FINAL) > 0){
  SOMATIC_LIKE <- FINAL[FINAL$P_VAL_adj < 0.05,]
...
# Fisher strand test
if (nrow(FINAL) > 0){
  FINAL$FISHER <- apply(FINAL,1,function(x) fisher.test(matrix(c(as.numeric(x["Ref_fwd"]), as.numeric(x["Alt_fwd"]), as.numeric(x["Ref_rev"]), as.numeric(x["Alt_rev"])), nrow = 2))[[1]])
  FINAL$FISHER <- p.adjust(FINAL$FISHER, method = 'fdr')
  } else {
    FINAL$FISHER <- numeric(0)
}
...
if (!is.null(ND)){
  ...
  # ND CALLING
  CALLING_ND <- CALLING_2(ND, PARAMS[PARAMS$BARCODE == 'ALL',])
  CALLING_ND <- CALLING_ND[CALLING_ND$POS %in% FINAL$POS,]
  
  if (nrow(CALLING_ND) > 0) {
    # Adjust P based on the amount of sites analysed
    if (!is.null(num_sites) | num_sites <= 0){
      NDn <- NT+NG
      CALLING_ND$nP_VAL_adj <- p.adjust(CALLING_ND$P_VAL, n = max(NDn, length(CALLING_ND$P_VAL)), method = "fdr")
    } else {
      CALLING_ND$nP_VAL_adj <- p.adjust(CALLING_ND$P_VAL, n = max(num_sites, length(CALLING_ND$P_VAL)), method = "fdr")
    }
  } else {
    CALLING_ND$nP_VAL_adj <- numeric(0)
  }
  
  # Keep only columns that we are interested
  nd <- CALLING_ND[,c("CHROM","POS", "DP_HQ","ALT_COUNT", "nP_VAL_adj")]
  ...
  CALLS <- merge(FINAL,nd_temp, by=c("CHROM","POS"),all.x = T)
} else {
  CALLS <- FINAL
}
...
# Ordering tsv file
CALLS <- CALLS[with(CALLS, order(CHROM, POS)),]

# Printing final table
write.table(x = CALLS, file = OUTF, quote = F, sep = '\t', col.names = T, row.names = F)