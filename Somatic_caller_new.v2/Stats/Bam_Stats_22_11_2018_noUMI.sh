...
$BEDTOOLS intersect -abam $BAM -b $BED2 | $SAMTOOLS sort -o $BAM2 - 
$SAMTOOLS index $BAM2

# Getting TSV
NAME2=$(echo $BAM2 | sed 's/bam$/tsv/')
$SAMTOOLS mpileup -d 9999999 -f $REF \
    -l $BED2 \
    -q $MQ \
    -Q 1 \
    -x \
    $BAM2 | python $SCRIPTS/pileup2tsv_08_01_19.py --pileup - -o $NAME2 --minBQ $BQ -m 10
...
# Error rates
echo Rscript $SCRIPTS/Plot_error_rate_22_11_2018_noUMI.R -bc1 $NAME2 \
    -out $MAIN_FOLDER/PLOTS
Rscript $SCRIPTS/Plot_error_rate_22_11_2018_noUMI.R -bc1 $NAME2 \
    -out $MAIN_FOLDER/PLOTS &
wait 
